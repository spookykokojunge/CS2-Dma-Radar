...
namespace Framework
{
    ...
    public sealed class LeechCore : IDisposable
    {
        ...
        public static unsafe LeechCore CreateFromConfig(ref LC_CONFIG pLcCreateConfig, out LC_CONFIG_ERRORINFO ConfigErrorInfo)
        {
            ...
            nint hLC = lci.LcCreateEx(ref pLcCreateConfig, out pLcErrorInfo);
            ConfigErrorInfo = new LC_CONFIG_ERRORINFO();
            ConfigErrorInfo.strUserText = "";
            if (pLcErrorInfo != nint.Zero && hLC != nint.Zero)
            {
            ...
            if (pLcErrorInfo != nint.Zero)
            {
                ...
                if (e.dwVersion == LC_CONFIG_ERRORINFO_VERSION)
                {
                    ...
                    if (e.cwszUserText > 0)
                    {
                        ConfigErrorInfo.strUserText = Marshal.PtrToStringUni((nint)(pLcErrorInfo.ToInt64() + cbERROR_INFO));
                    }
                }
        ...
        /// <returns>True if read successful, otherwise False.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public unsafe bool Read(ulong pa, uint cb, nint pb) =>
        ...
        public MEM_SCATTER[] ReadScatter(params ulong[] pas)
        {
            ...
            {
                vapMEM = Marshal.ReadIntPtr(new nint(vappMEMs + i * 8)).ToInt64();
                pMEM_qwA = new nint(vapMEM + 8);
            ...
            {
                pMEM = Marshal.ReadIntPtr(new nint(vappMEMs + i * 8));
        ...
        /// <returns>True if write successful, otherwise False. The write is best-effort and may fail. It's recommended to verify the write with a subsequent read.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public unsafe bool Write(ulong pa, uint cb, nint pb) =>
        ...
        public void WriteScatter(ref MEM_SCATTER[] MEMs)
        {
            int i;
            long vappMEMs, vapMEM;
            nint pMEM, pMEM_f, pMEM_qwA, pMEM_pb, pppMEMs;
            for (i = 0; i < MEMs.Length; i++)
            {
            ...
            {
                vapMEM = Marshal.ReadIntPtr(new nint(vappMEMs + i * 8)).ToInt64();
                pMEM_f = new nint(vapMEM + 4);
                pMEM_qwA = new nint(vapMEM + 8);
                pMEM_pb = Marshal.ReadIntPtr(new nint(vapMEM + 16));
        ...
        {
            byte[] bMemMap;
            if (Command(LC_CMD_MEMMAP_GET, null, out bMemMap) && bMemMap.Length > 0)
            {
        ...
        {
            return Command(LC_CMD_MEMMAP_SET, Encoding.UTF8.GetBytes(sMemMap), out byte[] bMemMap);
        }
    }
    ...
    public sealed class Vmm : IDisposable
    {
        ...
        private static unsafe nint Initialize(out LeechCore.LC_CONFIG_ERRORINFO ConfigErrorInfo, params string[] args)
        {
            nint pLcErrorInfo;
            int cbERROR_INFO = Marshal.SizeOf<lci.LC_CONFIG_ERRORINFO>();
            nint hVMM = VMMDLL_InitializeEx(args.Length, args, out pLcErrorInfo);
            ...
            if (e.dwVersion == LeechCore.LC_CONFIG_ERRORINFO_VERSION)
            {
                ...
                if (e.cwszUserText > 0)
                {
                    ConfigErrorInfo.strUserText = Marshal.PtrToStringUni((nint)(vaLcCreateErrorInfo + cbERROR_INFO));
                }
            }
            lci.LcMemFree(pLcErrorInfo);
            return hVMM;
        ...
        {
            return VMMDLL_ConfigGet(hVMM, fOption, out pqwValue);
        }

        public bool ConfigSet(ulong fOption, ulong qwValue)
        {
            return VMMDLL_ConfigSet(hVMM, fOption, qwValue);
        }
        ...
        public delegate bool VfsCallBack_AddFile(ulong h, [MarshalAs(UnmanagedType.LPUTF8Str)] string wszName, ulong cb, nint pExInfo);

        [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
        public delegate bool VfsCallBack_AddDirectory(ulong h, [MarshalAs(UnmanagedType.LPUTF8Str)] string wszName, nint pExInfo);

        public bool VfsList(string wszPath, ulong h, VfsCallBack_AddFile CallbackFile, VfsCallBack_AddDirectory CallbackDirectory)
        {
            VMMDLL_VFS_FILELIST FileList;
            ...
            return VMMDLL_VfsList(hVMM, wszPath, ref FileList);
        }
        ...
        public unsafe uint VfsRead(string wszFileName, uint cb, ulong cbOffset, out byte[] pbData)
        {
            ...
            fixed (byte* pb = data)
            {
                nt = VMMDLL_VfsRead(hVMM, wszFileName, pb, cb, out cbRead, cbOffset);
        ...
        public unsafe uint VfsWrite(string wszFileName, byte[] pbData, ulong cbOffset)
        {
            ...
            {
                return VMMDLL_VfsWrite(hVMM, wszFileName, pb, (uint)pbData.Length, out cbRead, cbOffset);
            }
        }
        ...
        {
            return VMMDLL_InitializePlugins(hVMM);
        }
        ...
        public unsafe MEM_SCATTER[] MemReadScatter(uint pid, uint flags, params ulong[] qwA)
        {
            ...
            VMMDLL_MemReadScatter(hVMM, pid, pppMEMs, (uint)MEMs.Length, flags);
            for (i = 0; i < MEMs.Length; i++)
            {
                pMEM = Marshal.ReadIntPtr(new nint(vappMEMs + i * 8));
                lci.LC_MEM_SCATTER n = Marshal.PtrToStructure<lci.LC_MEM_SCATTER>(pMEM);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public unsafe uint MemRead(uint pid, ulong qwA, uint cb, nint pb, uint flags = 0) =>
        ...
        {
            if (!VMMDLL_MemReadEx(hVMM, pid, qwA, (byte*)pb, cb, out var cbRead, flags))
                return 0;
            return cbRead;
        }
            ...
            where T : unmanaged
        {
            ...
            fixed (T* pb = &result)
            {
                if (!VMMDLL_MemReadEx(hVMM, pid, qwA, (byte*)pb, cb, out cbRead, flags))
                    return false;
            }
            if (cbRead != cb)
                return false;
        ...
        public unsafe bool MemPrefetchPages(uint pid, ulong[] qwA)
        {
            ...
            fixed (byte* pb = data)
            {
                return VMMDLL_MemPrefetchPages(hVMM, pid, pb, (uint)qwA.Length);
            }
        }
        ...
        /// <returns>True if write successful, otherwise False.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public unsafe bool MemWrite(uint pid, ulong qwA, uint cb, nint pb) =>
        ...
        public unsafe bool MemWrite(uint pid, ulong qwA, uint cb, void* pb) =>
            VMMDLL_MemWrite(hVMM, pid, qwA, (byte*)pb, cb);
            ...
            where T : unmanaged
        {
            uint cb = (uint)sizeof(T);
            byte* pb = (byte*)&value;
            return VMMDLL_MemWrite(hVMM, pid, qwA, pb, cb);
        }
            ...
            where T : unmanaged
        {
            ...
            fixed (T* pb = data)
            {
                return VMMDLL_MemWrite(hVMM, pid, qwA, (byte*)pb, cb);
            }
        }

        public bool MemVirt2Phys(uint dwPID, ulong qwVA, out ulong pqwPA)
        {
            return VMMDLL_MemVirt2Phys(hVMM, dwPID, qwVA, out pqwPA);
        }
        ...
        public unsafe ulong[] MemSearchM(uint pid, VMMDLL_MEM_SEARCHENTRY[] search, ulong vaMin = 0, ulong vaMax = 0xffffffffffffffff, uint cMaxResult = 0x10000, uint ReadFlags = 0)
        {
            ...
            VMMDLL_MEM_SEARCH_CONTEXT_SEARCHENTRY[] es = new VMMDLL_MEM_SEARCH_CONTEXT_SEARCHENTRY[16];
            ...
            VMMDLL_MEM_SEARCH_CONTEXT ctx = new VMMDLL_MEM_SEARCH_CONTEXT();
            ...
            uint pcva;
            nint ppva;
            if (!VMMDLL_MemSearch(hVMM, pid, ref ctx, out ppva, out pcva)) { return new ulong[0]; }
            ulong[] result = new ulong[pcva];
            for (int i = 0; i < pcva; i++)
            {
                result[i] = Marshal.PtrToStructure<ulong>(nint.Add(ppva, i * 8));
            }
            VMMDLL_MemFree((byte*)ppva.ToPointer());
            return result;
        }
        ...
        {
            return VMMDLL_PidGetFromName(hVMM, szProcName, out pdwPID);
        }
        ...
        public unsafe uint[] PidList()
        {
            bool result;
            ulong c = 0;
            result = VMMDLL_PidList(hVMM, null, ref c);
            if (!result || c == 0) { return new uint[0]; }
            fixed (byte* pb = new byte[c * 4])
            {
                result = VMMDLL_PidList(hVMM, pb, ref c);
                if (!result || c == 0) { return new uint[0]; }
                uint[] m = new uint[c];
                for (ulong i = 0; i < c; i++)
                {
                    m[i] = (uint)Marshal.ReadInt32((nint)(pb + i * 4));
                }
                return m;
            }
        }
        ...
        {
            bool result;
            ulong cbENTRY = (ulong)Marshal.SizeOf<VMMDLL_PROCESS_INFORMATION>();
            fixed (byte* pb = new byte[cbENTRY])
            {
                Marshal.WriteInt64(new nint(pb + 0), unchecked((long)VMMDLL_PROCESS_INFORMATION_MAGIC));
                Marshal.WriteInt16(new nint(pb + 8), unchecked((short)VMMDLL_PROCESS_INFORMATION_VERSION));
                result = VMMDLL_ProcessGetInformation(hVMM, pid, pb, ref cbENTRY);
                if (!result) { return new PROCESS_INFORMATION(); }
                VMMDLL_PROCESS_INFORMATION n = Marshal.PtrToStructure<VMMDLL_PROCESS_INFORMATION>((nint)pb);
                if (n.wVersion != VMMDLL_PROCESS_INFORMATION_VERSION) { return new PROCESS_INFORMATION(); }
        ...
        {
            byte* pb = VMMDLL_ProcessGetInformationString(hVMM, pid, fOptionString);
            if (pb == null) { return ""; }
            string s = Marshal.PtrToStringAnsi((nint)pb);
            VMMDLL_MemFree(pb);
            return s;
        }
        ...
        public unsafe IMAGE_DATA_DIRECTORY[] ProcessGetDirectories(uint pid, string wszModule)
        {
            ...
            bool result;
            uint cbENTRY = (uint)Marshal.SizeOf<VMMDLL_IMAGE_DATA_DIRECTORY>();
            fixed (byte* pb = new byte[16 * cbENTRY])
            {
                result = VMMDLL_ProcessGetDirectories(hVMM, pid, wszModule, pb);
                if (!result) { return new IMAGE_DATA_DIRECTORY[0]; }
                IMAGE_DATA_DIRECTORY[] m = new IMAGE_DATA_DIRECTORY[16];
                for (int i = 0; i < 16; i++)
                {
                    VMMDLL_IMAGE_DATA_DIRECTORY n = Marshal.PtrToStructure<VMMDLL_IMAGE_DATA_DIRECTORY>((nint)(pb + i * cbENTRY));
        ...
        public unsafe IMAGE_SECTION_HEADER[] ProcessGetSections(uint pid, string wszModule)
        {
            bool result;
            uint cData;
            uint cbENTRY = (uint)Marshal.SizeOf<VMMDLL_IMAGE_SECTION_HEADER>();
            result = VMMDLL_ProcessGetSections(hVMM, pid, wszModule, null, 0, out cData);
            if (!result || cData == 0) { return new IMAGE_SECTION_HEADER[0]; }
            fixed (byte* pb = new byte[cData * cbENTRY])
            {
                result = VMMDLL_ProcessGetSections(hVMM, pid, wszModule, pb, cData, out cData);
                if (!result || cData == 0) { return new IMAGE_SECTION_HEADER[0]; }
                IMAGE_SECTION_HEADER[] m = new IMAGE_SECTION_HEADER[cData];
                for (int i = 0; i < cData; i++)
                {
                    VMMDLL_IMAGE_SECTION_HEADER n = Marshal.PtrToStructure<VMMDLL_IMAGE_SECTION_HEADER>((nint)(pb + i * cbENTRY));
        ...
        {
            return VMMDLL_ProcessGetProcAddress(hVMM, pid, wszModuleName, szFunctionName);
        }

        public ulong ProcessGetModuleBase(uint pid, string wszModuleName)
        {
            return VMMDLL_ProcessGetModuleBase(hVMM, pid, wszModuleName);
        }
        ...
        public unsafe bool PdbLoad(uint pid, ulong vaModuleBase, out string szModuleName)
        {
            ...
            fixed (byte* pb = data)
            {
                bool result = VMMDLL_PdbLoad(hVMM, pid, vaModuleBase, pb);
                if (!result) { return false; }
        ...
        public unsafe bool PdbSymbolName(string szModule, ulong cbSymbolAddressOrOffset, out string szSymbolName, out uint pdwSymbolDisplacement)
        {
            ...
            fixed (byte* pb = data)
            {
                bool result = VMMDLL_PdbSymbolName(hVMM, szModule, cbSymbolAddressOrOffset, pb, out pdwSymbolDisplacement);
                if (!result) { return false; }
        ...
        {
            return VMMDLL_PdbSymbolAddress(hVMM, szModule, szSymbolName, out pvaSymbolAddress);
        }

        public bool PdbTypeSize(string szModule, string szTypeName, out uint pcbTypeSize)
        {
            return VMMDLL_PdbTypeSize(hVMM, szModule, szTypeName, out pcbTypeSize);
        }

        public bool PdbTypeChildOffset(string szModule, string szTypeName, string wszTypeChildName, out uint pcbTypeChildOffset)
        {
            return VMMDLL_PdbTypeChildOffset(hVMM, szModule, szTypeName, wszTypeChildName, out pcbTypeChildOffset);
        }
        ...
        {
            int cbMAP = Marshal.SizeOf<VMMDLL_MAP_PTE>();
            int cbENTRY = Marshal.SizeOf<VMMDLL_MAP_PTEENTRY>();
            nint pMap = nint.Zero;
            MAP_PTEENTRY[] m = new MAP_PTEENTRY[0];
            if (!VMMDLL_Map_GetPte(hVMM, pid, fIdentifyModules, out pMap)) { goto fail; }
            VMMDLL_MAP_PTE nM = Marshal.PtrToStructure<VMMDLL_MAP_PTE>(pMap);
            if (nM.dwVersion != VMMDLL_MAP_PTE_VERSION) { goto fail; }
            m = new MAP_PTEENTRY[nM.cMap];
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_PTEENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_PTEENTRY>((nint)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_PTEENTRY e;
                e.vaBase = n.vaBase;
        ...
        fail:
            VMMDLL_MemFree((byte*)pMap.ToPointer());
            return m;
        }

        public unsafe MAP_VADENTRY[] Map_GetVad(uint pid, bool fIdentifyModules = true)
        {
            int cbMAP = Marshal.SizeOf<VMMDLL_MAP_VAD>();
            int cbENTRY = Marshal.SizeOf<VMMDLL_MAP_VADENTRY>();
            nint pMap = nint.Zero;
            MAP_VADENTRY[] m = new MAP_VADENTRY[0];
            if (!VMMDLL_Map_GetVad(hVMM, pid, fIdentifyModules, out pMap)) { goto fail; }
            VMMDLL_MAP_VAD nM = Marshal.PtrToStructure<VMMDLL_MAP_VAD>(pMap);
            if (nM.dwVersion != VMMDLL_MAP_VAD_VERSION) { goto fail; }
            m = new MAP_VADENTRY[nM.cMap];
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_VADENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_VADENTRY>((nint)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_VADENTRY e;
                e.vaStart = n.vaStart;
                e.vaEnd = n.vaEnd;
        ...
        {
            int cbMAP = Marshal.SizeOf<VMMDLL_MAP_VADEX>();
            int cbENTRY = Marshal.SizeOf<VMMDLL_MAP_VADEXENTRY>();
            nint pMap = nint.Zero;
            MAP_VADEXENTRY[] m = new MAP_VADEXENTRY[0];
            if (!VMMDLL_Map_GetVadEx(hVMM, pid, oPages, cPages, out pMap)) { goto fail; }
            VMMDLL_MAP_VADEX nM = Marshal.PtrToStructure<VMMDLL_MAP_VADEX>(pMap);
            if (nM.dwVersion != VMMDLL_MAP_VADEX_VERSION) { goto fail; }
            m = new MAP_VADEXENTRY[nM.cMap];
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_VADEXENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_VADEXENTRY>((nint)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_VADEXENTRY e;
                e.tp = n.tp;
                e.iPML = n.iPML;
        ...
        {
            int cbMAP = Marshal.SizeOf<VMMDLL_MAP_MODULE>();
            int cbENTRY = Marshal.SizeOf<VMMDLL_MAP_MODULEENTRY>();
            nint pMap = nint.Zero;
            MAP_MODULEENTRY[] m = new MAP_MODULEENTRY[0];
            uint flags = fExtendedInfo ? (uint)0xff : 0;
            if (!VMMDLL_Map_GetModule(hVMM, pid, out pMap, flags)) { goto fail; }
            VMMDLL_MAP_MODULE nM = Marshal.PtrToStructure<VMMDLL_MAP_MODULE>(pMap);
            if (nM.dwVersion != VMMDLL_MAP_MODULE_VERSION) { goto fail; }
            m = new MAP_MODULEENTRY[nM.cMap];
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_MODULEENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_MODULEENTRY>((nint)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_MODULEENTRY e;
                ...
                {
                    VMMDLL_MAP_MODULEENTRY_DEBUGINFO nDbg = Marshal.PtrToStructure<VMMDLL_MAP_MODULEENTRY_DEBUGINFO>(n.pExDebugInfo);
                    eDbg.fValid = true;
                ...
                {
                    VMMDLL_MAP_MODULEENTRY_VERSIONINFO nVer = Marshal.PtrToStructure<VMMDLL_MAP_MODULEENTRY_VERSIONINFO>(n.pExVersionInfo);
                    eVer.fValid = true;
        ...
        public static ulong cbSize = 0x80000;

        public static bool FixCr3(Vmm vmm, uint GamePID, string module_name)
        {
            var gameAssembly = vmm.Map_GetModuleFromName(GamePID, module_name);
            ...
            try
            {
                ...
                string dtbData = Encoding.UTF8.GetString(bytes);
                ...
                {
                    ulong dtbValue;
                    ulong.TryParse(dtb, NumberStyles.HexNumber, null, out dtbValue);
                    vmm.ConfigSet(OPT_PROCESS_DTB | GamePID, dtbValue);
                    try
                    {
                        gameAssembly = vmm.Map_GetModuleFromName(GamePID, module_name);
                        return true;
                    }
                    catch { }
                }
        ...
        public unsafe MAP_MODULEENTRY Map_GetModuleFromName(uint pid, string wszModuleName)
        {
            ...
            if (!VMMDLL_Map_GetModuleFromName(hVMM, pid, wszModuleName, out pMap, 0)) { goto fail; }
            VMMDLL_MAP_MODULEENTRY nM = Marshal.PtrToStructure<VMMDLL_MAP_MODULEENTRY>(pMap);
            e.fValid = true;
            e.vaBase = nM.vaBase;
        ...
        {
            int cbMAP = Marshal.SizeOf<VMMDLL_MAP_UNLOADEDMODULE>();
            int cbENTRY = Marshal.SizeOf<VMMDLL_MAP_UNLOADEDMODULEENTRY>();
            nint pMap = nint.Zero;
            MAP_UNLOADEDMODULEENTRY[] m = new MAP_UNLOADEDMODULEENTRY[0];
            if (!VMMDLL_Map_GetUnloadedModule(hVMM, pid, out pMap)) { goto fail; }
            VMMDLL_MAP_UNLOADEDMODULE nM = Marshal.PtrToStructure<VMMDLL_MAP_UNLOADEDMODULE>(pMap);
            if (nM.dwVersion != VMMDLL_MAP_UNLOADEDMODULE_VERSION) { goto fail; }
            m = new MAP_UNLOADEDMODULEENTRY[nM.cMap];
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_UNLOADEDMODULEENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_UNLOADEDMODULEENTRY>((nint)(pMap.ToInt64() + cbMAP + i * cbENTRY));
        ...
        public unsafe MAP_EATENTRY[] Map_GetEAT(uint pid, string wszModule, out MAP_EATINFO EatInfo)
        {
            ...
            int cbMAP = Marshal.SizeOf<VMMDLL_MAP_EAT>();
            int cbENTRY = Marshal.SizeOf<VMMDLL_MAP_EATENTRY>();
            nint pMap = nint.Zero;
            MAP_EATENTRY[] m = new MAP_EATENTRY[0];
            if (!VMMDLL_Map_GetEAT(hVMM, pid, wszModule, out pMap)) { goto fail; }
            VMMDLL_MAP_EAT nM = Marshal.PtrToStructure<VMMDLL_MAP_EAT>(pMap);
            if (nM.dwVersion != VMMDLL_MAP_EAT_VERSION) { goto fail; }
            m = new MAP_EATENTRY[nM.cMap];
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_EATENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_EATENTRY>((nint)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_EATENTRY e;
        ...
        {
            int cbMAP = Marshal.SizeOf<VMMDLL_MAP_IAT>();
            int cbENTRY = Marshal.SizeOf<VMMDLL_MAP_IATENTRY>();
            nint pMap = nint.Zero;
            MAP_IATENTRY[] m = new MAP_IATENTRY[0];
            if (!VMMDLL_Map_GetIAT(hVMM, pid, wszModule, out pMap)) { goto fail; }
            VMMDLL_MAP_IAT nM = Marshal.PtrToStructure<VMMDLL_MAP_IAT>(pMap);
            if (nM.dwVersion != VMMDLL_MAP_IAT_VERSION) { goto fail; }
            m = new MAP_IATENTRY[nM.cMap];
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_IATENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_IATENTRY>((nint)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_IATENTRY e;
                e.vaFunction = n.vaFunction;
                e.wszFunction = n.wszFunction;
        ...
        {
            nint pMap = nint.Zero;
            int cbMAP = Marshal.SizeOf<VMMDLL_MAP_HEAP>();
            int cbENTRY = Marshal.SizeOf<VMMDLL_MAP_HEAPENTRY>();
            int cbSEGENTRY = Marshal.SizeOf<VMMDLL_MAP_HEAPSEGMENTENTRY>();
            MAP_HEAP Heap;
            Heap.heaps = new MAP_HEAPENTRY[0];
            Heap.segments = new MAP_HEAPSEGMENTENTRY[0];
            if (!VMMDLL_Map_GetHeap(hVMM, pid, out pMap)) { goto fail; }
            VMMDLL_MAP_HEAP nM = Marshal.PtrToStructure<VMMDLL_MAP_HEAP>(pMap);
            if (nM.dwVersion != VMMDLL_MAP_HEAP_VERSION) { goto fail; }
            Heap.heaps = new MAP_HEAPENTRY[nM.cMap];
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_HEAPENTRY nH = Marshal.PtrToStructure<VMMDLL_MAP_HEAPENTRY>((nint)(pMap.ToInt64() + cbMAP + i * cbENTRY));
            ...
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_HEAPSEGMENTENTRY nH = Marshal.PtrToStructure<VMMDLL_MAP_HEAPSEGMENTENTRY>((nint)(nM.pSegments.ToInt64() + i * cbSEGENTRY));
        ...
        {
            nint pHeapAllocMap = nint.Zero;
            int cbMAP = Marshal.SizeOf<VMMDLL_MAP_HEAPALLOC>();
            int cbENTRY = Marshal.SizeOf<VMMDLL_MAP_HEAPALLOCENTRY>();
            if (!VMMDLL_Map_GetHeapAlloc(hVMM, pid, vaHeapOrHeapNum, out pHeapAllocMap)) { return new MAP_HEAPALLOCENTRY[0]; }
            VMMDLL_MAP_HEAPALLOC nM = Marshal.PtrToStructure<VMMDLL_MAP_HEAPALLOC>(pHeapAllocMap);
            if (nM.dwVersion != VMMDLL_MAP_HEAPALLOC_VERSION)
            {
                VMMDLL_MemFree((byte*)pHeapAllocMap.ToPointer());
                return new MAP_HEAPALLOCENTRY[0];
            }
            MAP_HEAPALLOCENTRY[] m = new MAP_HEAPALLOCENTRY[nM.cMap];
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_HEAPALLOCENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_HEAPALLOCENTRY>((nint)(pHeapAllocMap.ToInt64() + cbMAP + i * cbENTRY));
                m[i].va = n.va;
                m[i].cb = n.cb;
                m[i].tp = n.tp;
            }
        ...
        {
            int cbMAP = Marshal.SizeOf<VMMDLL_MAP_THREAD>();
            int cbENTRY = Marshal.SizeOf<VMMDLL_MAP_THREADENTRY>();
            nint pMap = nint.Zero;
            MAP_THREADENTRY[] m = new MAP_THREADENTRY[0];
            if (!VMMDLL_Map_GetThread(hVMM, pid, out pMap)) { goto fail; }
            VMMDLL_MAP_THREAD nM = Marshal.PtrToStructure<VMMDLL_MAP_THREAD>(pMap);
            if (nM.dwVersion != VMMDLL_MAP_THREAD_VERSION) { goto fail; }
            m = new MAP_THREADENTRY[nM.cMap];
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_THREADENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_THREADENTRY>((nint)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_THREADENTRY e;
                e.dwTID = n.dwTID;
                e.dwPID = n.dwPID;
        ...
        {
            int cbMAP = Marshal.SizeOf<VMMDLL_MAP_HANDLE>();
            int cbENTRY = Marshal.SizeOf<VMMDLL_MAP_HANDLEENTRY>();
            nint pMap = nint.Zero;
            MAP_HANDLEENTRY[] m = new MAP_HANDLEENTRY[0];
            if (!VMMDLL_Map_GetHandle(hVMM, pid, out pMap)) { goto fail; }
            VMMDLL_MAP_HANDLE nM = Marshal.PtrToStructure<VMMDLL_MAP_HANDLE>(pMap);
            if (nM.dwVersion != VMMDLL_MAP_HANDLE_VERSION) { goto fail; }
            m = new MAP_HANDLEENTRY[nM.cMap];
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_HANDLEENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_HANDLEENTRY>((nint)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_HANDLEENTRY e;
        ...
        {
            int cbMAP = Marshal.SizeOf<VMMDLL_MAP_NET>();
            int cbENTRY = Marshal.SizeOf<VMMDLL_MAP_NETENTRY>();
            nint pMap = nint.Zero;
            MAP_NETENTRY[] m = new MAP_NETENTRY[0];
            if (!VMMDLL_Map_GetNet(hVMM, out pMap)) { goto fail; }
            VMMDLL_MAP_NET nM = Marshal.PtrToStructure<VMMDLL_MAP_NET>(pMap);
            if (nM.dwVersion != VMMDLL_MAP_NET_VERSION) { goto fail; }
            m = new MAP_NETENTRY[nM.cMap];
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_NETENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_NETENTRY>((nint)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_NETENTRY e;
                e.dwPID = n.dwPID;
                e.dwState = n.dwState;
                e.dwPoolTag = n.dwPoolTag;
        ...
        {
            int cbMAP = Marshal.SizeOf<VMMDLL_MAP_PHYSMEM>();
            int cbENTRY = Marshal.SizeOf<VMMDLL_MAP_PHYSMEMENTRY>();
            nint pMap = nint.Zero;
            MAP_PHYSMEMENTRY[] m = new MAP_PHYSMEMENTRY[0];
            if (!VMMDLL_Map_GetPhysMem(hVMM, out pMap)) { goto fail; }
            VMMDLL_MAP_PHYSMEM nM = Marshal.PtrToStructure<VMMDLL_MAP_PHYSMEM>(pMap);
            if (nM.dwVersion != VMMDLL_MAP_PHYSMEM_VERSION) { goto fail; }
            m = new MAP_PHYSMEMENTRY[nM.cMap];
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_PHYSMEMENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_PHYSMEMENTRY>((nint)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_PHYSMEMENTRY e;
                e.pa = n.pa;
                e.cb = n.cb;
                m[i] = e;
        ...
        public unsafe MAP_POOLENTRY[] Map_GetPool()
        {
            byte[] tag = { 0, 0, 0, 0 };
            nint pN = nint.Zero;
            int cbMAP = Marshal.SizeOf<VMMDLL_MAP_POOL>();
            int cbENTRY = Marshal.SizeOf<VMMDLL_MAP_POOLENTRY>();
            if (!VMMDLL_Map_GetPool(hVMM, out pN, 0)) { return new MAP_POOLENTRY[0]; }
            VMMDLL_MAP_POOL nM = Marshal.PtrToStructure<VMMDLL_MAP_POOL>(pN);
            if (nM.dwVersion != VMMDLL_MAP_POOL_VERSION)
            {
                VMMDLL_MemFree((byte*)pN.ToPointer());
                return new MAP_POOLENTRY[0];
            }
            MAP_POOLENTRY[] eM = new MAP_POOLENTRY[nM.cMap];
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_POOLENTRY nE = Marshal.PtrToStructure<VMMDLL_MAP_POOLENTRY>((nint)(pN.ToInt64() + cbMAP + i * cbENTRY));
                eM[i].va = nE.va;
                eM[i].cb = nE.cb;
        ...
        {
            int cbMAP = Marshal.SizeOf<VMMDLL_MAP_USER>();
            int cbENTRY = Marshal.SizeOf<VMMDLL_MAP_USERENTRY>();
            nint pMap = nint.Zero;
            MAP_USERENTRY[] m = new MAP_USERENTRY[0];
            if (!VMMDLL_Map_GetUsers(hVMM, out pMap)) { goto fail; }
            VMMDLL_MAP_USER nM = Marshal.PtrToStructure<VMMDLL_MAP_USER>(pMap);
            if (nM.dwVersion != VMMDLL_MAP_USER_VERSION) { goto fail; }
            m = new MAP_USERENTRY[nM.cMap];
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_USERENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_USERENTRY>((nint)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_USERENTRY e;
                e.szSID = n.wszSID;
                e.wszText = n.wszText;
        ...
        {
            int cbMAP = Marshal.SizeOf<VMMDLL_MAP_SERVICE>();
            int cbENTRY = Marshal.SizeOf<VMMDLL_MAP_SERVICEENTRY>();
            nint pMap = nint.Zero;
            MAP_SERVICEENTRY[] m = new MAP_SERVICEENTRY[0];
            if (!VMMDLL_Map_GetServices(hVMM, out pMap)) { goto fail; }
            VMMDLL_MAP_SERVICE nM = Marshal.PtrToStructure<VMMDLL_MAP_SERVICE>(pMap);
            if (nM.dwVersion != VMMDLL_MAP_SERVICE_VERSION) { goto fail; }
            m = new MAP_SERVICEENTRY[nM.cMap];
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_SERVICEENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_SERVICEENTRY>((nint)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_SERVICEENTRY e;
                e.vaObj = n.vaObj;
                e.dwPID = n.dwPID;
                e.dwOrdinal = n.dwOrdinal;
        ...
        public unsafe MAP_PFNENTRY[] Map_GetPfn(params uint[] pfns)
        {
            bool result;
            uint cbPfns;
            int cbMAP = Marshal.SizeOf<VMMDLL_MAP_PFN>();
            int cbENTRY = Marshal.SizeOf<VMMDLL_MAP_PFNENTRY>();
            ...
            fixed (byte* pbPfns = dataPfns)
            {
                ...
                {
                    result =
                        VMMDLL_Map_GetPfn(hVMM, pbPfns, (uint)pfns.Length, null, ref cbPfns) &&
                        VMMDLL_Map_GetPfn(hVMM, pbPfns, (uint)pfns.Length, pb, ref cbPfns);
                    if (!result) { return new MAP_PFNENTRY[0]; }
                    VMMDLL_MAP_PFN pm = Marshal.PtrToStructure<VMMDLL_MAP_PFN>((nint)pb);
                    if (pm.dwVersion != VMMDLL_MAP_PFN_VERSION) { return new MAP_PFNENTRY[0]; }
                    MAP_PFNENTRY[] m = new MAP_PFNENTRY[pm.cMap];
                    for (int i = 0; i < pm.cMap; i++)
                    {
                        VMMDLL_MAP_PFNENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_PFNENTRY>((nint)(pb + cbMAP + i * cbENTRY));
        ...
        public unsafe REGISTRY_HIVE_INFORMATION[] RegHiveList()
        {
            bool result;
            uint cHives;
            int cbENTRY = Marshal.SizeOf<VMMDLL_REGISTRY_HIVE_INFORMATION>();
            result = VMMDLL_WinReg_HiveList(hVMM, null, 0, out cHives);
            if (!result || cHives == 0) { return new REGISTRY_HIVE_INFORMATION[0]; }
            fixed (byte* pb = new byte[cHives * cbENTRY])
            {
                result = VMMDLL_WinReg_HiveList(hVMM, pb, cHives, out cHives);
                ...
                {
                    VMMDLL_REGISTRY_HIVE_INFORMATION n = Marshal.PtrToStructure<VMMDLL_REGISTRY_HIVE_INFORMATION>((nint)(pb + i * cbENTRY));
                    REGISTRY_HIVE_INFORMATION e;
                    if (n.wVersion != VMMDLL_REGISTRY_HIVE_INFORMATION_VERSION) { return new REGISTRY_HIVE_INFORMATION[0]; }
        ...
        public unsafe byte[] RegHiveRead(ulong vaCMHIVE, uint ra, uint cb, uint flags = 0)
        {
            ...
            fixed (byte* pb = data)
            {
                if (!VMMDLL_WinReg_HiveReadEx(hVMM, vaCMHIVE, ra, pb, cb, out cbRead, flags))
                {
                    return null;
                }
            }
        ...
        public unsafe bool RegHiveWrite(ulong vaCMHIVE, uint ra, byte[] data)
        {
            fixed (byte* pb = data)
            {
                return VMMDLL_WinReg_HiveWrite(hVMM, vaCMHIVE, ra, pb, (uint)data.Length);
            }
        }
        ...
        public unsafe REGISTRY_ENUM RegEnum(string wszFullPathKey)
        {
            ...
            fixed (byte* pb = new byte[0x1000])
            {
                i = 0;
                cchName = 0x800;
                while (VMMDLL_WinReg_EnumKeyExW(hVMM, wszFullPathKey, i, pb, ref cchName, out ftLastWriteTime))
                {
                ...
                i = 0;
                cchName = 0x800;
                while (VMMDLL_WinReg_EnumValueW(hVMM, wszFullPathKey, i, pb, ref cchName, out lpType, null, ref cbData))
                {
        ...
        public unsafe byte[] RegValueRead(string wszFullPathKeyValue, out uint tp)
        {
            bool result;
            uint cb = 0;
            result = VMMDLL_WinReg_QueryValueExW(hVMM, wszFullPathKeyValue, out tp, null, ref cb);
            if (!result)
            {
                return null;
            }
            ...
            fixed (byte* pb = data)
            {
                result = VMMDLL_WinReg_QueryValueExW(hVMM, wszFullPathKeyValue, out tp, pb, ref cb);
                return result ? data : null;
            }
        }
    }
    ...
    public sealed class VmmScatter : IDisposable
    {
            ...
            where T : unmanaged
        {
            ...
            fixed (T* pb = &result)
            {
                if (!VMMDLL_Scatter_Read(hS, qwA, cb, (byte*)pb, out cbRead))
                    return false;
            }
            if (cbRead != cb)
                return false;
        ...
        {
            return VMMDLL_Scatter_Prepare(hS, qwA, cb);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public unsafe bool PrepareWrite(ulong qwA, byte[] data) =>
            PrepareWriteArray(qwA, data);
            ...
            where T : unmanaged
        {
            ...
            fixed (T* pb = data)
            {
                return VMMDLL_Scatter_PrepareWrite(hS, qwA, (byte*)pb, cb);
            }
        }
            ...
            where T : unmanaged
        {
            uint cb = (uint)sizeof(T);
            byte* pb = (byte*)&value;
            return VMMDLL_Scatter_PrepareWrite(hS, qwA, pb, cb);
        }
    ...
    internal static class lci
    {
        ...
        public static extern nint LcCreate(ref LeechCore.LC_CONFIG pLcCreateConfig);

        [DllImport("leechcore", EntryPoint = "LcCreateEx")]
        public static extern nint LcCreateEx(ref LeechCore.LC_CONFIG pLcCreateConfig, out nint ppLcCreateErrorInfo);

        [DllImport("leechcore", EntryPoint = "LcClose")]
        internal static extern void LcClose(nint hLC);

        [DllImport("leechcore", EntryPoint = "LcMemFree")]
        internal static extern unsafe void LcMemFree(nint pv);

        [DllImport("leechcore", EntryPoint = "LcAllocScatter1")]
        internal static extern unsafe bool LcAllocScatter1(uint cMEMs, out nint pppMEMs);

        [DllImport("leechcore", EntryPoint = "LcRead")]
        internal static extern unsafe bool LcRead(nint hLC, ulong pa, uint cb, byte* pb);

        [DllImport("leechcore", EntryPoint = "LcReadScatter")]
        internal static extern unsafe void LcReadScatter(nint hLC, uint cMEMs, nint ppMEMs);

        [DllImport("leechcore", EntryPoint = "LcWrite")]
        internal static extern unsafe bool LcWrite(nint hLC, ulong pa, uint cb, byte* pb);

        [DllImport("leechcore", EntryPoint = "LcWriteScatter")]
        internal static extern unsafe void LcWriteScatter(nint hLC, uint cMEMs, nint ppMEMs);

        [DllImport("leechcore", EntryPoint = "LcGetOption")]
        public static extern bool GetOption(nint hLC, ulong fOption, out ulong pqwValue);

        [DllImport("leechcore", EntryPoint = "LcSetOption")]
        public static extern bool SetOption(nint hLC, ulong fOption, ulong qwValue);

        [DllImport("leechcore", EntryPoint = "LcCommand")]
        internal static extern unsafe bool LcCommand(nint hLC, ulong fOption, uint cbDataIn, byte* pbDataIn, out nint ppbDataOut, out uint pcbDataOut);
    }
    ...
    internal static class vmmi
    {
        ...
        internal static extern nint VMMDLL_InitializeEx(
            int argc,
            string[] argv,
            out nint ppLcErrorInfo);
        ...
        internal struct VMMDLL_VFS_FILELIST
        {
            ...
            internal nint pfnAddFile;
            internal nint pfnAddDirectory;
            internal ulong h;
        }
        ...
        public static extern bool VMMDLL_InitializePlugins(nint hVMM);
        ...
        internal static extern unsafe nint VMMDLL_Scatter_Initialize(
            nint hVMM,
            uint dwPID,
            uint flags);
        ...
        public static extern bool SVMMDLL_Scatter_Clear(nint hS, uint dwPID, uint flags);
        ...
        internal static extern unsafe bool VMMDLL_PidList(nint hVMM, byte* pPIDs, ref ulong pcPIDs);

        [DllImport("vmm", EntryPoint = "VMMDLL_PidGetFromName")]
        public static extern bool VMMDLL_PidGetFromName(nint hVMM, [MarshalAs(UnmanagedType.LPStr)] string szProcName, out uint pdwPID);

        [DllImport("vmm", EntryPoint = "VMMDLL_ProcessGetProcAddressW")]
        public static extern ulong VMMDLL_ProcessGetProcAddress(nint hVMM, uint pid, [MarshalAs(UnmanagedType.LPWStr)] string wszModuleName, [MarshalAs(UnmanagedType.LPStr)] string szFunctionName);

        [DllImport("vmm", EntryPoint = "VMMDLL_ProcessGetModuleBaseW")]
        public static extern ulong VMMDLL_ProcessGetModuleBase(nint hVMM, uint pid, [MarshalAs(UnmanagedType.LPWStr)] string wszModuleName);
        ...
        internal struct VMMDLL_MAP_HEAPALLOC
        {
            internal uint dwVersion;
            [MarshalAs(UnmanagedType.ByValArray, SizeConst = 7)] internal uint[] _Reserved1;
            internal nint _Reserved20;
            internal nint _Reserved21;
            internal uint cMap;
        }
        ...
        internal struct VMMDLL_MEM_SEARCH_CONTEXT
        {
            ...
            internal nint pvUserPtrOpt;
            internal nint pfnResultOptCB;
            internal ulong ReadFlags;
            internal bool fForcePTE;
            internal bool fForceVAD;
            internal nint pfnFilterOptCB;
        }
